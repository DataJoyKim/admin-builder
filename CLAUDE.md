...
2. Load its `WorkflowFunction` nodes plus the `WorkflowEdge` rows that connect them, and assemble both into a `WorkflowGraph`. Nodes are identified by `nodeId` (not the DB id, which changes on every save since the console rewrites all nodes); edges are `sourceNodeId` → `targetNodeId` with a `BranchType`: `DEFAULT` for ordinary flow, `CASE` (plus a `branchId`) for one branch of a condition, `ELSE` for its fallback branch. Execution walks the graph from the start node (the one with no incoming edge); a node with no outgoing edge ends the flow. `orderNum` is only the builder's display order and never decides what runs next.
3. Each node names a `FunctionType` (`ENTITY`, `SQL`, `REST_CLIENT`, `MESSAGE_PROCESSOR`, `NOTIFICATION`, `CONDITION`) and reads/writes a slot in a shared `Map<String, List<Map<String,Object>>>` message store keyed by `requestMessageId`/`responseMessageId` — this is how steps pass data to each other. `FunctionFactory.instance(FunctionType)` resolves a node to a `FunctionExecutor` implementation (`function/executor/*`); each executor's `execute(user, functionName, params)` does the real work and returns a `FunctionResult` (success/failure). `CONDITION` is the exception: it produces no data. Its branches are `WorkflowCondition` rows (one per `if` / `else if`, ordered by `orderNum`, each with a `branchId` that ties it to its `CASE` edge). `WorkflowService` evaluates them in order through `ConditionEvaluator` (JS via `executor/script`, with `params` = the node's IN slot), follows the `CASE` edge of the first one that is true, and falls back to the `ELSE` edge when none match — nodes on the branches not taken never run and never appear in the response.
...
The workflow builder console screen (`templates/console/contents/workflow-builder.html`) is a free-form canvas: nodes are placed at `WorkflowFunction.positionX/positionY` (builder-only values, ignored at execution time) and every connection is drawn by dragging a node's output port onto another node, so two branches can converge on one shared node instead of duplicating it. Nodes without a stored position (saved before the canvas existed) are auto-laid-out on load, and a workflow with nodes but no edges is read as an `orderNum` straight line so old definitions migrate on the next save.

When adding a new kind of workflow step: add a `FunctionType`, an executor implementing `FunctionExecutor`, and wire it into `FunctionFactory`. Control-flow nodes like `CONDITION` are the exception — they are handled in `WorkflowService`'s traversal, not by an executor.
