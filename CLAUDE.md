...
When adding a new kind of workflow step: add a `FunctionType`, an executor implementing `NodeExecutor`, and wire it into `NodeExecutorFactory`. Control-flow nodes like `CONDITION` are the exception — they are handled in `WorkflowService`'s traversal, not by an executor.
