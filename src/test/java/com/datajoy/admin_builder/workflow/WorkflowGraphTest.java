...
import com.datajoy.admin_builder.function.ConditionEvaluator;
...
class WorkflowGraphTest {
    ...
    private WorkflowFunction node(String nodeId, FunctionType functionType, int orderNum) {
    ...
    public void 연결선을_따라서만_흐른다() {
        WorkflowFunction first = node("n1", FunctionType.SQL, 1);
        WorkflowFunction second = node("n2", FunctionType.SQL, 2);
    ...
    public void 조건은_판정순서대로_들어오고_각_가지는_branchId로_이어진다() {
        ...
        WorkflowFunction branch = node("n2", FunctionType.CONDITION, 2);
        WorkflowFunction ifNode = node("n3", FunctionType.ENTITY, 3);
        WorkflowFunction elseIfNode = node("n4", FunctionType.SQL, 4);
        WorkflowFunction elseNode = node("n5", FunctionType.NOTIFICATION, 5);
