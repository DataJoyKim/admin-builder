...
public class WorkflowGraph {
    ...
    public static WorkflowGraph of(List<WorkflowFunction> functions, List<WorkflowEdge> edges) {
    ...
    public static WorkflowGraph of(
        ...
        List<WorkflowFunction> orderedFunctions = sortByOrderNum(functions);

        Map<String, WorkflowFunction> nodes = new LinkedHashMap<>();
        for(WorkflowFunction function : orderedFunctions) {
    ...
    public WorkflowFunction getStartNode() {
        return startNode;
    }
     ...
     */
    public List<WorkflowCondition> conditionsOf(WorkflowFunction node) {
     ...
     * 참이 된 조건의 가지.
     */
    public WorkflowFunction nextCase(WorkflowFunction node, String branchId) {
     ...
     */
    public WorkflowFunction nextElse(WorkflowFunction node) {
        for(WorkflowEdge edge : edgesOf(node)) {
    ...
    public WorkflowFunction next(WorkflowFunction current, BranchType branchType) {
    ...
    private List<WorkflowEdge> edgesOf(WorkflowFunction node) {
    ...
    private static WorkflowFunction findStartNode(Map<String, WorkflowFunction> nodes, Set<String> targetNodeIds) {
        // 시작 노드는 들어오는 연결이 없는 노드다. 순환 등으로 후보가 없으면 순서상 첫 노드로 시작한다.
        for(Map.Entry<String, WorkflowFunction> node : nodes.entrySet()) {
    ...
    private static List<WorkflowFunction> sortByOrderNum(List<WorkflowFunction> functions) {
        List<WorkflowFunction> orderedFunctions = new ArrayList<>(functions == null ? List.of() : functions);

        orderedFunctions.sort(Comparator
                .comparing(WorkflowFunction::getOrderNum, Comparator.nullsLast(Comparator.naturalOrder()))
                .thenComparing(WorkflowFunction::getId, Comparator.nullsLast(Comparator.naturalOrder())));
